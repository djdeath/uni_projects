...
void
hash_table_clear (HashTable_t *ht)
{
	if (ht)
	{
		if (ht->free)
...
vhash_table_new (unsigned int size, HashMode_t mode, va_list args)
{
	...
	if (ht)
	{
		...
		case HASH_MODE_OPEN:
			...
			ht->remove = hash_table_open_remove;
...
int
hash_table_remove (HashTable_t *ht, const void *key, void *data)
{
	int ret;
...
void
hash_table_foreach (HashTable_t *ht, Foreach_t func, void *udata)
{
	if (!ht || !func)
		return;
